...
const tabs = ["List of Our Investors", "Impact Story of Series1", "Dividend Payment History", "Our Price"];
    ...
    <div>
      ...
      <section className="py-16 md:py-24">
        <div className="section-container">
          <h2 className="font-heading font-bold text-2xl text-foreground mb-4 uppercase tracking-wider">Our Investment Information</h2>
              ...
              {tabs.map((tab, i) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
                    activeTab === i ? "cef-gradient-bg text-primary-foreground" : "bg-muted text-foreground hover:bg-muted/80"
                  }`}
                >
                  {tab}
                </button>
                      ...
                      {investors.map((inv, i) => (
                        <tr key={inv.name} className={i % 2 === 0 ? "bg-muted" : "bg-background"}>
                          <td className="px-4 py-3">
                            <div className="w-16 h-10 bg-muted-foreground/10 rounded flex items-center justify-center text-xs text-muted-foreground">Logo</div>
                          </td>
                ...
                <div className="bg-muted rounded-xl p-10 flex items-center justify-center min-h-[300px]">
                  <div className="text-center">
                    <div className="w-full max-w-lg mx-auto aspect-video bg-foreground/10 rounded-lg flex items-center justify-center mb-4">
                      <span className="text-muted-foreground text-sm">Video Placeholder – CEF Impact Story</span>
                    </div>
                    <p className="text-sm text-muted-foreground">CEF Impact Story Pass 4 1</p>
                  </div>
                </div>
              )}
